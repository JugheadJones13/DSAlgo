...
public:
    int ans;
    void callR(vector<int>& rating,vector<int> curr){
        int n = curr.size();
        if (n == 3){
            if (check(rating,curr)) ans++;
            return;
        }
        if (n == 0){
            ...
            for (int i = last+1; i <= rating.size() - 3 + n; i++){
                vector<int> t1 = curr;
                t1.push_back(i);
                callR(rating,t1);
            }
    ...
    bool check(vector<int>& rating,vector<int> curr){
        int x = rating[curr[0]];
        int y = rating[curr[1]];
        int z = rating[curr[2]];
        bool ans = false;
        if (x > y && y > z) ans = true;
        else if (x < y && y < z) ans = true;
        else;
        return ans;
    }
